...
/// This represents 'node' clause in the '#pragma oss ...' directive.
///
/// \code
/// #pragma oss task node(foo(N))
/// \endcode
/// In this example directive '#pragma oss task' has simple 'node'
/// clause with expression 'foo(N)'.
class OSSNodeClause : public OSSClause {
  friend class OSSClauseReader;

  /// Location of '('.
  SourceLocation LParenLoc;

  /// Expression of the 'node' clause.node
  Stmt *Expression = nullptr;
...
public:
  /// Build 'node' clause with expression \a E.
  ///
  /// \param StartLoc Starting location of the clause.
  /// \param LParenLoc Location of '('.
  /// \param E Expression of the clause.
  /// \param EndLoc Ending location of the clause.
  OSSNodeClause(Expr *E, SourceLocation StartLoc, SourceLocation LParenLoc,
                SourceLocation EndLoc)
      : OSSClause(llvm::oss::OSSC_node, StartLoc, EndLoc), LParenLoc(LParenLoc),
        Expression(E) {}

  /// Build an empty clause.
  OSSNodeClause()
      : OSSClause(llvm::oss::OSSC_node, SourceLocation(), SourceLocation()) {}
  ...
  static bool classof(const OSSClause *T) {
    return T->getClauseKind() == llvm::oss::OSSC_node;
  }
};
